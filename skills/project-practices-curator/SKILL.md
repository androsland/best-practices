...
6. Write proposed knowledge updates through `scripts/curation_state.py`. A video claim first becomes a candidate or supporting revision. It may become enforceable only in a separate reviewed revision backed by primary authority and appropriate applicability/confidence.
...
- `--new-only` is based on stable source IDs in local state, not filenames. Dry-run never mutates state or downloads media.
